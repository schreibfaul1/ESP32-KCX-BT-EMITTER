...
//  log_i("%s", m_chbuf);
    ...
    if(startsWith(m_chbuf, "POWER ON"))             { cmd_PowerOn();       m_Answ =  2; ie = 0; goto P2;}
    if(startsWith(m_chbuf, "OK+BT"))                { cmd_Mode();          m_Answ =  3; ie = 0; goto P2;}
    if(startsWith(m_chbuf, "Auto_link_Add:"))       { cmd_AutoLink();      m_Answ = -1; ie = 0; goto P1;}
    if(startsWith(m_chbuf, "OK+VOL"))               { cmd_Volume();        m_Answ =  4; ie = 0; goto P2;}
    if(startsWith(m_chbuf, "Delete_Vmlink"))        { cmd_Delete();        m_Answ =  5; ie = 0; goto P2;}
    if(startsWith(m_chbuf, "BT_ADD_NUM"))           { cmd_AddNum();        m_Answ = -1; ie = 0; goto exit;}
    if(startsWith(m_chbuf, "BT_NAME_NUM"))          { cmd_NameNum();       m_Answ = -1; ie = 0; goto exit;}
    if(startsWith(m_chbuf, "MEM_Name"))             { cmd_MemName();       m_Answ = -1; ie = 0; goto P0;}
    if(startsWith(m_chbuf, "MEM_MacAdd"))           { cmd_MemAddr();       m_Answ = -1; ie = 0; goto P0;}
    if(startsWith(m_chbuf, "MacAdd"))               { cmd_scannedItems();  m_Answ = -1; ie = 0; goto exit;}
    if(startsWith(m_chbuf, "OK+NAME"))              { cmd_connectedName(); m_Answ =  6; ie = 0; goto P2;}
...
void KCX_BT_Emitter::responseError(){
    if(kcx_bt_info) kcx_bt_info("no response: last command was:", m_lastCommand);
}
...
void KCX_BT_Emitter::cmd_Wrong(){
    if(kcx_bt_info) kcx_bt_info("wrong command:", m_lastCommand);
}
...
void KCX_BT_Emitter::addQueueItem(const char* item){
    ...
    m_messageQueue.insert(m_messageQueue.begin(), x_ps_strdup(item));
}

const char* KCX_BT_Emitter::getQueueItem(){
    if(m_queueItem) {free(m_queueItem), m_queueItem = NULL;}
    if(m_messageQueue.size() == 0) return NULL;
    m_queueItem = x_ps_strdup(m_messageQueue[m_messageQueue.size() - 1]);
    free(m_messageQueue[m_messageQueue.size() - 1]);
    m_messageQueue[m_messageQueue.size() - 1] = NULL;
    m_messageQueue.pop_back();
    m_messageQueue.shrink_to_fit();
    return m_queueItem;
}
